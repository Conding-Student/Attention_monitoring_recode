...
export default function JoinMeetingPage({ initialMeetingValue = "" }: JoinMeetingPageProps) {
  const { meetingValue, error, handleMeetingValueChange, joinMeeting } = useJoinMeeting({
    initialMeetingValue,
  });
  ...
  return (
    <main className="relative min-h-[calc(100dvh-72px)] bg-transparent">
      <section className="relative mx-auto flex min-h-[calc(100dvh-72px)] max-w-[940px] flex-col items-center justify-center px-4 py-5 sm:px-6 sm:py-7">
        ...
        <div className="w-full max-w-[760px] rounded-[24px] border border-white/70 bg-white/75 px-5 py-7 shadow-[0_24px_70px_rgba(15,23,42,0.12)] backdrop-blur-xl transition-colors sm:px-8 sm:py-8 dark:border-slate-700/60 dark:bg-slate-900/70 dark:shadow-[0_24px_70px_rgba(0,0,0,0.4)]">
          <header className="mb-7 text-center">
            ...
            <h1 className="text-2xl font-black tracking-[-0.03em] text-[#303234] sm:text-3xl dark:text-slate-100">Join a Meeting</h1>

            <p className="mt-1.5 text-sm text-gray-500 dark:text-slate-400">Enter a meeting code or link to join</p>
          </header>
          ...
          >
            <div>
              <label htmlFor="meetingCodeOrLink" className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-slate-300">
              ...
              <div className="relative">
                <input
                  ...
                  type="text"
                  value={meetingValue}
                  onChange={(event) => handleMeetingValueChange(event.target.value)}
                ...
                <Link2 aria-hidden="true" size={17} className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500" />
              </div>
            </div>
