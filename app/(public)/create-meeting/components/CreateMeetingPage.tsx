...
export default function CreateMeetingPage({ initialMeetingCode }: CreateMeetingPageProps) {
  const { meetingName, meetingCode, displayShareLink, copiedField, error, handleMeetingNameChange, copyMeetingCode, copyShareLink, startMeeting } = useCreateMeeting({
    initialMeetingCode,
  });
      ...
      <section className="relative mx-auto flex min-h-[calc(100dvh-72px)] max-w-[940px] flex-col items-center justify-center px-4 py-5 sm:px-6 sm:py-7">
        ...
        <div className="w-full max-w-[760px] rounded-[24px] border border-white/70 bg-white/75 px-5 py-6 shadow-[0_24px_70px_rgba(15,23,42,0.12)] backdrop-blur-xl transition-colors sm:px-8 sm:py-7 dark:border-slate-700/60 dark:bg-slate-900/70 dark:shadow-[0_24px_70px_rgba(0,0,0,0.4)]">
          <header className="mb-6 text-center">
            <h1 className="text-2xl font-black tracking-[-0.03em] text-[#303234] sm:text-3xl dark:text-slate-100">Create a Meeting</h1>

            <p className="mt-1.5 text-sm text-gray-500 dark:text-slate-400">Set up your meeting space and invite your team</p>
          </header>
          ...
          >
            <div>
              <label htmlFor="meetingName" className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-slate-300">
                Meeting Name
              </label>
              ...
              <input
                ...
                type="text"
                value={meetingName}
                onChange={(event) => handleMeetingNameChange(event.target.value)}
            ...
            <div>
              <span className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-slate-300">Meeting Code</span>
              ...
              <div className="flex min-h-[78px] flex-col gap-3 rounded-xl border border-gray-200 bg-white/65 px-4 py-3 backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between sm:px-5 dark:border-slate-700 dark:bg-slate-950/45">
                <span className="break-all font-mono text-2xl font-black tracking-[0.05em] text-[#303234] sm:text-3xl dark:text-slate-100">{meetingCode}</span>
            ...
            <div>
              <span className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-slate-300">Share Link</span>
              ...
              <div className="flex h-12 items-center overflow-hidden rounded-xl border border-gray-200 bg-white/85 transition-all hover:border-gray-300 focus-within:border-[#07594b] focus-within:ring-4 focus-within:ring-emerald-100/70 dark:border-slate-700 dark:bg-slate-950/65 dark:hover:border-slate-600 dark:focus-within:border-emerald-500 dark:focus-within:ring-emerald-950">
                <div className="min-w-0 flex-1 px-4">
                  <p className="truncate text-sm text-gray-600 dark:text-slate-300">{displayShareLink}</p>
                </div>
                ...
                >
                  {copiedField === "link" ? <Check size={16} /> : <Link2 size={16} />}

                  <span className="hidden sm:inline">{copiedField === "link" ? "Copied" : "Copy Link"}</span>
                </button>
              </div>
            </div>
        ...
        <footer className="mt-5 text-center text-[11px] text-gray-400 sm:text-xs dark:text-slate-500">© 2026 Bakawan Data Analytics · Professional Video Communication</footer>
      </section>
    </main>
  );
}
